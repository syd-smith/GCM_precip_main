...
def fut_mean(model_name, variable, interpolate = False):
    ...
    if variable == 'tos':
        fpath = fpath = f'/uufs/chpc.utah.edu/common/home/strong-group7/sydney/GSLBIP/ERA5/{variable}/{variable}_Omon_'
    ...
    open_fut = xr.open_mfdataset(fpath + model_name + '_ssp*.nc', engine = 'netcdf4')
    ...
    if interpolate == True:
        ...
        ds_out = xr.Dataset(
            {
                "lat": (["lat"], np.arange(15.62, 51.88, 1.25)),
                "lon": (["lon"], np.arange(216.5625, 295.275, 1.875)),
            }
        )

        regridder = xe.Regridder(open_fut, ds_out, 'bilinear')
...
combine = xr.concat(data, dim = 'model')
...
def anomaly(model_name, variable, interpolate = False):
    ...
    open_hist = xr.open_mfdataset(fpath + model_name + '_hist*.nc', engine = 'netcdf4')
    open_fut = xr.open_mfdataset(fpath + model_name + '_ssp*.nc', engine = 'netcdf4')
    ...
    if interpolate == True:
        ...
        regridder_hist = xe.Regridder(open_hist, ds_out, 'bilinear')
        regridder_fut = xe.Regridder(open_fut, ds_out, 'bilinear')
...
def map_anomalies(anomaly_ref, data, variable, udata = None, vdata = None, interpolate = False, quiver = False, step = 1):
    ...
    if variable == 'psl': 
        plot_dict = {'cmap_a' : cmap.cmap('MPL_coolwarm'),
                     'cmap_m' : cmap.cmap('MPL_coolwarm'),
                     'title': 'Sea Level Pressure', 
                     'cbar_a' : 'Change in Sea Level Pressure (hPa)',
                     'cbar_m' : 'Mean Sea Level Pressure (hPa)'
                     }
    elif variable == 'pr':
        plot_dict = {'cmap_a' : cmap.cmap('MPL_BrBG'),
                     'cmap_m' : cmap.cmap('MPL_BrBG'),
                     'title': 'Precipitation',
        ...
        plot_dict = {'cmap_a' : cmap.cmap('CBR_wet'),
                     'cmap_m' : cmap.cmap('MPL_coolwarm'),
        ...
        plot_dict = {'cmap_a' : cmap.cmap('cmp_b2r'),
                     'cmap_m' : cmap.cmap('amwg256'),
                     'title': 'Test Variable',
    ...
    if data_min < 0.00 < data_max:
        norm = mcolors.TwoSlopeNorm(vmin=data_min, vcenter=0.00, vmax=data_max)
    else:
    ...
    if anomaly_ref.__name__ == 'fut_mean':
        contour = ax.contourf(data['lon'], data['lat'], data.values, cmap = plot_dict['cmap_m'], transform = ccrs.PlateCarree(), levels = 20, norm = norm)
        ax.set_title(f'Wet Models ssp585\nMean {plot_dict['title']}\n2070-2099', fontsize = 18)
        cbar = plt.colorbar(contour, orientation = 'horizontal', pad = 0.03, aspect = 50, shrink = 0.85)
        ...
        contour = ax.contourf(data['lon'], data['lat'], data.values, cmap = plot_dict['cmap_a'], transform = ccrs.PlateCarree(), levels = 20, norm = norm)
    ...
    # save path for files
    save_path = '/uufs/chpc.utah.edu/common/home/strong-group7/sydney/GSLBIP/variable_mapping/test_maps/'
