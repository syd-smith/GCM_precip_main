...
m1 = axs[0].pcolormesh(GCM_mean['lon'].values, GCM_mean['lat'].values, GCM_mean.values, cmap = cmap.cmap('MPL_BrBG'))
...
m2 = axs[1].pcolormesh(MACA_mean['lon'].values, MACA_mean['lat'].values, MACA_mean.values, cmap = cmap.cmap('MPL_BrBG'))
...
for ax in axs:
    ...
    # add VIC boundaries
    box = mpatches.Rectangle((min_lon, min_lat), (max_lon - min_lon), (max_lat - min_lat), linewidth = 2.5, edgecolor = 'red', facecolor = 'none', zorder = 5)
    ax.add_patch(box)
