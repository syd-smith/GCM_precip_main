...
from lon_conversion import convert_lon_360_to_180, convert_lon_to_0_360
...
def gridmet_mean_yr(variable):
    ...
    if variable == 'pr':
        ...
        gridmet = xr.open_dataset(gridmet_fpath)
        ...
        gridmet_min = xr.open_dataset(gridmet_fpath_min)
        ...
        gridmet_max = xr.open_dataset(gridmet_fpath_max)
        ...
        for year in range(1979, 2015):
            for month in range(6, 9):
                ...
                combined_mean = xr.concat([dat_mean_max, dat_mean_min], dim = 'M')
...
def gridmet_mean_overall(variable):
    ...
    if variable == 'pr':
        ...
        combined_mean = xr.concat([obs_max, obs_min], dim = 'M')
...
def mean_JJA_overall(model_name, variable):
    #open historical dataset for model
    fpath = fpath = f'/uufs/chpc.utah.edu/common/home/strong-group7/sydney/GSLBIP/ERA5/{variable}/{variable}_Amon_'
    open_hist = xr.open_mfdataset(fpath + model_name + '_hist*.nc')
...
def heat_map_bias(bias_matrix, name, variable):
    ...
    bias_array = np.array(bias_matrix)
    vmin = float(np.min(bias_array))
    vmax = float(np.max(bias_array))
