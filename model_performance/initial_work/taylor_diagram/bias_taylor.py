...
gcm_dict = read_file('gcm_dict_dec_3.txt', '/uufs/chpc.utah.edu/common/home/strong-group7/sydney/GSLBIP/model_performance/taylor_diagram')

pr_levels = np.array([])
temp_levels = np.array([])
for model in MACA_models:
    pr_levels = np.append(pr_levels, gcm_dict[model]['pr_ratio'])
    temp_levels = np.append(temp_levels, gcm_dict[model]['tasmax_change'])
...
def scaled_data(variable):
    """
    Scaled data to be used in arccos function.
    """
    
    # add all data points for the called variable to a numpy array to determine max and min
    values = np.array([])
    for model in  MACA_models:
        values = np.append(values, gcm_dict[model]['summer_bias'][variable])

    a = values.min()
    b = values.max()

    if a < 0:
        scaled_bias = np.zeros_like(values, dtype=float)
...
def bias_taylor(variable_list):
    ...
    fig, axs = plt.subplots(1, 2, subplot_kw={'projection': 'polar'}, figsize = (11, 8))
    
    one, two = axs
    
    pr_levels = np.array([50, 65, 80, 95, 100, 105, 150, 190, 220])
    temp_levels = np.array([3, 4, 5, 6, 7, 8, 9])
    
    pr_norm = mcolors.BoundaryNorm(pr_levels, ncolors = plt.get_cmap(cmap.cmap('MPL_BrBG'), 10).N) # 15 bins means 14 edges
    ts_norm = mcolors.BoundaryNorm(temp_levels, ncolors = plt.get_cmap(cmap.cmap('BlRe'), 6).N) # 15 bins means 14 edges
    ...
    for index, diagram in enumerate([one, two]):
        ...
        diagram.plot(np.pi - np.arccos(obs_bias), obs_stdev_ratio, marker = 'o', markerfacecolor =  'none', markeredgecolor = 'k', label = "Reference", markersize = 12)
    ...
    pr_cbar.ax.xaxis.set_major_formatter(ticker.FormatStrFormatter('%.0f'))
    
    temp_cbar = plt.colorbar(ts, 
                               orientation = 'horizontal', 
                               ticks = temp_levels, 
                               shrink = 0.75, 
                               pad = pad,
    ...
    temp_cbar.ax.xaxis.set_major_formatter(ticker.FormatStrFormatter('%.0f'))
