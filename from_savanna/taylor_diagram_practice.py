...
def mask_MACA(model_name, start_month, stop_month, save = False):
    ...
    ds_open = bp.xr.open_dataset(fpath + model_name + '_ssp585_pr.nc', engine = "netcdf4", decode_times = time_coder)
...
def st_dev_obs(start_month, stop_month):
    ...
    ds = bp.xr.open_dataset(f'/uufs/chpc.utah.edu/common/home/strong-group7/sydney/data_analysis/taylor_plot/masked/st_dev_gridmet_{start_month}-{stop_month}_masked.nc')
...
def corcoef(model_name, start_month, stop_month):
    ...
    # open observational data and model dataset
    ds_gridmet = bp.xr.open_dataset(f'/uufs/chpc.utah.edu/common/home/strong-group7/sydney/data_analysis/taylor_plot/masked/st_dev_gridmet_{start_month}-{stop_month}_masked.nc')
   
    # raises error if specified file wasn't created above using masking functions
    if not bp.os.path.isfile(ds_gridmet):
    ...
    # opens saved xarray from masking functions above
    fpath = f'/uufs/chpc.utah.edu/common/home/strong-group7/sydney/data_analysis/taylor_plot/masked/GCM_{model_name}_{start_month}-{stop_month}_masked.nc'
    ds_MACA = bp.xr.open_dataset(fpath)
    
    # raises error if specified file wasn't created above using masking functions
    if not bp.os.path.isfile(ds_MACA):
    ...
    corcoef = bp.xr.corr(ds_gridmet, ds_MACA, dim = 'time')
...
def bias(model_name, start_month, stop_month):
    ...
    # opens saved xarray from masking functions above
    fpath = f'/uufs/chpc.utah.edu/common/home/strong-group7/sydney/data_analysis/taylor_plot/masked/GCM_{model_name}_{start_month}-{stop_month}_masked.nc'
    ds_model = bp.xr.open_dataset(fpath)
    
    # raises error if specified file wasn't created above using masking functions
    if not bp.os.path.isfile(ds_model):
